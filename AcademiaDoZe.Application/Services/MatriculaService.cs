...
public class MatriculaService : IMatriculaService
{
    ...
    private static DateOnly CalcularDataFinal(DateOnly dataInicio, AppMatriculaPlano plano) => plano switch
    {
    ...
    public async Task<MatriculaDto?> ObterPorIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        return matricula?.ToDto(matricula.Aluno.ToDto());
    }
    ...
    public async Task<IEnumerable<MatriculaDto>> ObterTodasAsync(CancellationToken cancellationToken = default)
    {
        ...
        return [.. matriculas.Select(m => m.ToDto(m.Aluno.ToDto()))];
    }
    ...
    public async Task<bool> PossuiMatriculaAtivaAsync(int alunoId, CancellationToken cancellationToken = default)
        => await _matriculaRepoFactory().PossuiMatriculaAtiva(alunoId, cancellationToken);
    ...
    public async Task<MatriculaDto> AdicionarAsync(MatriculaDto matriculaDto, CancellationToken cancellationToken = default)
    {
        ...
        var dataFinal = CalcularDataFinal(matriculaDto.DataInicio, matriculaDto.Plano);
        var matricula = matriculaDto.ToEntity(aluno, dataFinal);
    ...
    public async Task<MatriculaDto> AtualizarAsync(MatriculaDto matriculaDto, CancellationToken cancellationToken = default)
    {
        ...
        var aluno = matriculaExistente.Aluno;
        ...
        var dataInicio = matriculaDto.DataInicio != default ? matriculaDto.DataInicio : matriculaExistente.DataInicio;
        var plano = matriculaDto.Plano != default ? matriculaDto.Plano : matriculaExistente.Plano.ToApplication();
        var dataFinal = CalcularDataFinal(dataInicio, plano);

        var matriculaAtualizada = matriculaExistente.UpdateFromDto(matriculaDto, aluno, dataFinal);
