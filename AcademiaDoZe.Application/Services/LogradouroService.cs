...
public class LogradouroService : ILogradouroService
{
    ...
    public async Task<bool> CepJaExisteAsync(string cep, int? id = null, CancellationToken cancellationToken = default)
    {
        ...
        if (!cepResult.Sucesso) return false;
        return await _repoFactory().CepJaExiste(cepResult.Valor!, id, cancellationToken);
    }
    ...
    public async Task<LogradouroDto?> ObterPorCepAsync(string cep, CancellationToken cancellationToken = default)
    {
        ...
        if (!cepResult.Sucesso)
            throw new ArgumentException($"CEP inválido: {string.Join(", ", cepResult.Notificacoes.Select(n => n.Mensagem))}", nameof(cep));

        var logradouro = await _repoFactory().ObterPorCep(cepResult.Valor!, cancellationToken);
        return logradouro?.ToDto();
    }
    ...
    public async Task<LogradouroDto> AdicionarAsync(LogradouroDto logradouroDto, CancellationToken cancellationToken = default)
    {
        ...
        if (!cepResult.Sucesso)
            throw new ArgumentException($"CEP inválido: {string.Join(", ", cepResult.Notificacoes.Select(n => n.Mensagem))}", nameof(logradouroDto));

        if (await _repoFactory().CepJaExiste(cepResult.Valor!, null, cancellationToken))
    ...
    public async Task<LogradouroDto> AtualizarAsync(LogradouroDto logradouroDto, CancellationToken cancellationToken = default)
    {
        ...
        if (await _repoFactory().CepJaExiste(cepResult.Valor!, logradouroDto.Id, cancellationToken))
