...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
        ...
        string nomeColumn = "nome")
    {
        try
        {
            ...
            Arquivo? foto =
                fotoBytes != null
                    ? Arquivo.Criar("foto", ".jpg", fotoBytes).Valor
                    : null;

            var logradouro =
                LogradouroRepository.Map(
                    reader,
                    "logradouro_nome");
            ...
            if (!enderecoResult.Sucesso)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro ao criar endereço do colaborador ID {id}: " +
                    $"{string.Join(", ", enderecoResult.Notificacoes.Select(n => n.Mensagem))}");
            }
                ...
                !senhaResult.Sucesso)
            {
                throw new InfrastructureException(
                    "ERRO_DOMINIO_MAPEAMENTO",
                    $"Erro ao criar ValueObjects do colaborador ID {id}.");
            }
            ...
            if (!result.Sucesso)
            {
                throw new InfrastructureException(
                    ...
                    $"{string.Join(", ", result.Notificacoes.Select(n => n.Mensagem))}");
            }
            ...
            string query =
                FormatInsertQuery(
                    ...
                    (cpf, nome, nascimento, telefone, email,
                     logradouro_id, numero, complemento, senha,
                     foto, admissao, tipo, vinculo)
                    VALUES
                    (@Cpf, @Nome, @Nascimento, @Telefone, @Email,
                     @LogradouroId, @Numero, @Complemento, @Senha,
                     @Foto, @Admissao, @Tipo, @Vinculo)");
