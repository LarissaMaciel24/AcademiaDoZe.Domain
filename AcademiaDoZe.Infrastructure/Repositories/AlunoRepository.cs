        ...
        try
        {
            ...
            if (!enderecoResult.Sucesso)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro ao criar endereço do aluno ID {id}: " +
                ...
                !senhaResult.Sucesso)
            {
                throw new InfrastructureException(
                    "ERRO_DOMINIO_MAPEAMENTO",
                    $"Erro ao criar ValueObjects do aluno ID {id}.");
            }
        ...
        try
        {
            string query =
                FormatInsertQuery(
                    ...
                    VALUES
                    (@Cpf, @Nome, @Nascimento, @Telefone, @Email,
                     @LogradouroId, @Numero, @Complemento, @Senha, @Foto)");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException(
                ...
                $"Erro ao obter aluno por Email: {ex.Message}",
                ex);
        }
    }
